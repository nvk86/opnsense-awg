...
class PeerController extends ApiMutableModelControllerBase
{
    ...
    private function buildClientConfig(string $uuid,$peer):array
    {
        ...
        foreach(['jc','jmin','jmax','s1','s2','s3','s4','h1','h2','h3','h4','i1','i2','i3','i4','i5'] as $f){$v=trim((string)$server->$f);if($v!=='')$rows[]=strtoupper(substr($f,0,1)).substr($f,1).' = '.$v;}
